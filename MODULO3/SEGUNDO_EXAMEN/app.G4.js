...
const inmobiliaria = {
  ...
  alquilar: function (id){
    ...
    console.log(`departamento con id: ${id} esta alquilado` );
  ...
  filtrarPetFriendly: function (){
    ...
    for (let i = 0; i < this.departamentos.length; i++) {
      if (this.departamentos[i].aceptaMascotas) {
        dtoPetFriendly.push(this.departamentos[i])

      }
      
    }
    return dtoPetFriendly;
  },
  rebajasPorNoAlquiler: function(){
    let dtosDisponibles = inmobiliaria.departamentosDisponibles()
    for (let i = 0; i < dtosDisponibles.length; i++) {
      dtosDisponibles[i].precioAlquiler *= 0.9

      

      
    }
    return dtosDisponibles
